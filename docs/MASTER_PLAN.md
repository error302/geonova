...
| ID | Title | Status | Files | Effort | Notes |
|----|-------|--------|-------|--------|-------|
| P2-1 | Resume Phase 13 from Milestone A (canonical submission domain model) | in progress (Milestone B manifest done) | `src/lib/submission/manifest.ts` (new), `src/lib/submission/assembleSubmission.ts`, `src/types/submission.ts` (SUBMISSION_SECTIONS), migration 047 | large (2-4 weeks) | **Milestone A complete**: submission model + numbering + surveyor identity. **Milestone B (package skeleton)**: added `manifest.ts` — `evaluatePackageCompleteness` + `buildPackageManifest` map generated artifacts onto the 8 benchmark `SUBMISSION_SECTIONS` and report ready/missing-required/missing-optional in benchmark order; `assembleSubmission.ts` now embeds the section report + completeness in `manifest.json`. 7 unit tests. **Remaining**: Workstreams 5-9 (Form No. 4 title-block/coordinate-table upgrade, computation workbook sheet ordering, shapefile .prj export, package assembler polish). **Known schema drift (needs live-DB reconcile)**: code writes `submission_sequences` (migration 007) while canonical `numbering.ts` reads `submission_sequence` (migration 047); `surveyor_profiles` has `isk_number` but `surveyorProfile.ts` queries `registration_number`. |
