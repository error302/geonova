...
| ID | Title | Status | Files | Effort | Notes |
|----|-------|--------|-------|--------|-------|
| P2-1 | Resume Phase 13 from Milestone A (canonical submission domain model) | in progress (Milestone A done) | New `src/lib/db/migrations/047_project_submissions.sql` (2 tables + PL/pgSQL function + RLS), `src/types/submission.ts` (SurveyorProfile added), `src/lib/submission/surveyorProfile.ts` (getActiveSurveyorProfile), `src/lib/submission/numbering.ts` (generateSubmissionNumber + incrementRevision + parseSubmissionNumber), 13 tests | large (2-4 weeks) | **Milestone A complete**: Workstream 1 (project_submissions + submission_sequence tables with atomic sequence increment via PL/pgSQL), Workstream 2 (getActiveSurveyorProfile — DB-backed identity, replaces localStorage), Workstream 3 (SRVY2025-1 numbering format [RegNo]_[YYYY]_[###]_[R##] with atomic increment + revision bumping + parsing). 13 unit tests passing. **Remaining**: Workstreams 4-10 (canonical workspace route, Form No. 4 upgrade, computation workbook generator, package assembler, shapefile export with .prj, DWG→DXF guidance UI). Pre-existing type errors in assembleSubmission.ts and preview/route.ts need camelCase→snake_case property fix — those files were written speculatively before the SurveyorProfile type existed. |
