...
- **Uncommitted (this batch):** explicit-any grind batch 2 — 10 files (dataCleaner `res.json()` typed to `CleanDataResponse`, leveling.test + ntrip-client.test typed mock WebSocket/global casts, engineering/compute `ReturnType<typeof createClient>` + `new Array<number>` + `unknown` params, useCollaboration real `YArrayEvent<unknown>` observer + typed awareness local state, MotionComponents GlassCard+GlowButton typed motion spreads dropping motion-conflicting handlers, templates/index `Record<string, unknown>` + typed regex callback, computationWorker generic `sendRequest<T>`, profileSvg + WorkerBridge `: any` removal). Floors explicit-any **189**, member-access **336**, assignment **492**, argument 0, total **3,297**.
...
### Phase 2 — `no-explicit-any` (538 → 0, 227 files) — **batch 1 + batch 2 done (floor 348 → 189)**

Batch 1 (earlier session) ground the top-density cluster — `vectorTileFactory.ts` + its two test files, `deedPlanExport.test.ts`, `leveling-standards.ts`, `governmentLicensing.ts`, `CrossSectionInput.tsx` — 68 explicit-any → 0. Batch 2 (this session) took the next 10-file cluster to zero: `dataCleaner`, `leveling.test`, `engineering/compute`, `ntrip-client.test`, `useCollaboration`, `MotionComponents`, `templates/index`, `computationWorker`, `profileSvg`, `WorkerBridge` — 45 explicit-any → 0 (plus member-access −20, assignment −7, unsafe-call −5, unsafe-return −4). Recipe: real yjs `YArrayEvent<unknown>` observer types + typed awareness local state, generic `sendRequest<T>` for the worker bridge, `Record<string, unknown>` template params, typed motion-component spreads (dropping the animation/drag handlers React types conflict with framer-motion's `AnimationDefinition` on), `ReturnType<typeof createClient>` via a type-only import, and typed mock WebSocket/global casts in tests. Floor explicit-any **189**. Next: regen the ranking (`warn-scan` / eslint aggregation) and grind the next cluster — top files were `LayerControl.tsx` + `parcelVault.ts` (11 each) at last check, though earlier OL batches already drained LayerControl.
