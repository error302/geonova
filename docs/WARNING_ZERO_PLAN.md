...
| `no-unused-vars` | **788** | warn | ⏳ next |
| `no-non-null-assertion` | **0** | `error` | ✅ done |
| `react-hooks/exhaustive-deps` | **27** | warn | ⏳ |
| `no-restricted-syntax` | **16** | warn | ⏳ |
| **total** | **848** | CI ceiling **1,000** | green |

Order = finish the family closest to zero first (each finish removes a floor + shrinks the ceiling), then mechanical rules, then CI tightening. **All six `no-unsafe-*` families are done** — only the mechanical rules remain. Finish order: **unused-vars (788) → exhaustive-deps (27) → no-restricted-syntax (16) → ceiling → 0**. (ED-1 2026-08-13: 17 sites across admin/payments, analytics, MapClient, mobile/field, progress-monitor, LongSectionRenderer, FieldBookMobile, WeatherPanel, StakeoutRadar — useCallback-wrapped loaders/fetchWeather hoisted above calling effects to avoid TDZ, mapInstance/editingToolRef captured in the cleanup closure, progress-monitor tolerance wrapped in useMemo, duplicate dep removed.)
...
| `react-hooks/exhaustive-deps` | **27** | careful per-effect dep fixes; `eslint-disable-next-line` only with a justification (repo convention) | next: 37 single-site files (useMapInit, useMapState, useViewportQuery, NavBar, CADEditor, …) |
