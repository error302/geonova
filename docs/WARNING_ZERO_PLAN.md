...
**Progress:** 14,030 → **1,380** (measured 2026-08-12, `lint-ratchets --report` on origin/main). Row-typing (0/538), a11y (0 findings), member-access, explicit-any, argument, **assignment**, no-unsafe-return and no-unsafe-call are **done** (rules flipped to `error`); `no-unused-vars`/`no-non-null-assertion` + the mechanical rules remain.
...
| `no-non-null-assertion` | 214 | replace `!` with real narrowing/guards (tests: `defined()` helper + assert; prod: type-predicate filters / destructure guards) | next tier per live scan (batch 5 done: 6-warning tier, 9 files; 214 live) |
