...
**Progress:** 14,030 → **1,304** (measured 2026-08-12, `lint-ratchets --report` on origin/main). Row-typing (0/538), a11y (0 findings), member-access, explicit-any, argument, **assignment**, no-unsafe-return and no-unsafe-call are **done** (rules flipped to `error`); `no-unused-vars`/`no-non-null-assertion` + the mechanical rules remain.
...
| `no-non-null-assertion` | 189 | replace `!` with real narrowing/guards (tests: `defined()` helper + assert; prod: type-predicate filters / destructure guards) | next tier per live scan (batch 6 done: 5-warning tier; 189 live) |
...
3. Tighten `--max-warnings`: 20,000 → 10,000 → 5,000 → 3,700 → 3,000 → 2,800 → 2,000 → **1,500 (now)** → 500 → **0** as the totals shrink. Keep the ceiling documented in the workflow files (see the earlier 10k tightening for the pattern).
