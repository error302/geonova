...
- **Uncommitted (this batch):** explicit-any grind batch 3 — 11 files (useFieldBook row-typing recipe: `FieldBookEntryRow` interface + typed `data` cast clearing ~10 unsafe warnings; totalStation/boundaryLaw/easementLaw/kenyaSurveyStandards/fieldplan/cloudRendering typed-array callbacks; realtime/index dead `on*Change` callbacks → `unknown` (no callers); profile/page string[]/literal callbacks; PerformanceMonitor `PerformanceEventTiming` + structural CLS casts; leveling-standards.test typed callbacks). Floors explicit-any **156**, member-access **298**, assignment **478**, argument 0, total **3,193**.
