...
let run
if (runArg) {
  const info = JSON.parse(gh(['run', 'view', runArg, '--json', 'databaseId,status,headSha,createdAt,conclusion,displayTitle']))
  run = info
} else {
...
// ── 3. Parse each shard's list-reporter log ────────────────────────────────
...
for (const shard of shards) {
  ...
  for (const t of tests.values()) {
    const run = t.tests
    const passN = run.filter((x) => x.passed).length
    const failN = run.length - passN
    const flakyN = run.filter((x) => x.failedOnce && x.passed).length
    if (failN > 0) failCount += failN
    const agg = perSpec.get(t.spec) || { tests: 0, pass: 0, fail: 0, flaky: 0, seconds: 0 }
    agg.tests += run.length
    agg.pass += passN
    agg.fail += failN
    agg.flaky += flakyN
    agg.seconds += run[run.length - 1].dur // final attempt's duration
...
const totalTests = perShard.reduce((s, p) => s + p.perSpec.reduce((a, x) => a + x[1].tests, 0), 0)
const totalFail = perShard.reduce((s, p) => s + p.failCount, 0)
const totalWall = perShard.reduce((s, p) => s + (p.wallSec ?? 0), 0)
const totalTest = perShard.reduce((s, p) => s + p.testSec, 0)

console.log(`run ${run.databaseId} (ci.yml, ${(run.createdAt || '').slice(0, 10)}, sha ${(run.headSha || '').slice(0, 7)}${runConc}) — ${shards.length} shards, ${totalTests} tests (${totalTests - totalFail} passed${totalFail ? `, ${totalFail} failed` : ''}), test time ${fmtTime(totalTest)}, wall ${fmtTime(totalWall)}`)
console.log()
