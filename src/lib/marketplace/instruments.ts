...
export function getListings(filters?: {
  ...
  let items = loadListings().filter((l: InstrumentListing) => !l.sold)
    .sort((a: InstrumentListing, b: InstrumentListing) => b.postedAt.localeCompare(a.postedAt))
...
export function searchListings(q: string): InstrumentListing[] {
  ...
  return loadListings().filter((l: InstrumentListing) => !l.sold && (
...
export function deleteListing(id: string) {
  saveListings(loadListings().filter((l: InstrumentListing) => l.id !== id))
}
...
export function getInquiriesFor(listingId: string): InquiryMessage[] {
  return loadInquiries().filter((m: InquiryMessage) => m.listingId === listingId)
    .sort((a: InquiryMessage, b: InquiryMessage) => b.sentAt.localeCompare(a.sentAt))
}
