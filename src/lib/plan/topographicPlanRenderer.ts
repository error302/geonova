...
export function renderTopographicPlan(opts: TopographicPlanOptions): string {
  ...
  // ⚡ Bolt: Replace map() and spread operator with single loop to prevent
  // V8 "Maximum call stack size exceeded" errors on large datasets and avoid intermediate allocations.
