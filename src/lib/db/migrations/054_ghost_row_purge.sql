...
DELETE FROM project_fieldbook_entries WHERE trim(coalesce(station,'')) = '' AND trim(coalesce(point_id,'')) = '' AND coalesce(bs,'')='' AND coalesce(fs,'')='' AND coalesce(slope_dist,'')='' AND coalesce(slope_distance,'')='';
