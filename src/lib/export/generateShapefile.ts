...
export async function generateShapefileZip(
  ...
  if (beacons.length > 0) {
    const pointsGeoJSON = {
      ...
      features: beacons.map((b: BeaconData) => ({
  ...
  if (boundaries.length > 0) {
    const linesGeoJSON = {
      type: 'FeatureCollection' as const,
      features: boundaries.map((b: BoundaryLine) => ({
        type: 'Feature' as const,
        geometry: {
  ...
  if (parcels.length > 0) {
    const polygonsGeoJSON = {
      type: 'FeatureCollection' as const,
      features: parcels.map((p: ParcelData) => ({
        type: 'Feature' as const,
        geometry: {
          type: 'Polygon' as const,
          coordinates: [p.boundaryPoints.map((pt: { easting: number; northing: number }) => [pt.easting, pt.northing])]
        },
        properties: {
          PARCEL_ID: p.id,
