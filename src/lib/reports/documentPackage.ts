...
export const DOCS_BY_TYPE: Record<string, DocumentDef[]> = {
  ...
  traverse: [
    ...
    { id: 'control_submission','title': 'Control Survey Report (Form C22)', description: 'Submit new control points to Survey of Kenya for registration', required: false },
  ],
  leveling: [
    ...
    { id: 'leveling_summary',  title: 'Levelling Summary',             description: 'Reduced levels, arithmetic check, misclosure, correction',      required: true },
  ...
  engineering: [
    ...
    { id: 'completion_certificate','title': 'Certificate of Completion', description: 'Surveyor certifies setout is complete and within tolerance',      required: true },
    { id: 'as_built_certificate',  title: 'As-built certificate',        description: 'As-built survey confirming constructed positions',                required: false },
  ],
  stakeout: [
    ...
    { id: 'completion_certificate','title': 'Certificate of Completion', description: 'Surveyor certifies pegs placed within tolerance',                 required: true },
  ],
  gnss_baseline: [
    ...
    { id: 'control_submission','title': 'Control Survey Report',       description: 'Submit new GNSS control to national network (if applicable)',    required: false },
  ],
  mining: [
...
export function generateFieldNotes(
  ...
  const typeLabel = (project.survey_type || 'Survey').charAt(0).toUpperCase() + (project.survey_type || 'survey').slice(1)
  const controlPts = points.filter((p: PointData) => p.is_control)
  const detailPts  = points.filter((p: PointData) => !p.is_control)
...
export function generateLevelingSummary(
  ...
  const ex = extraFields || {}
  const elevationPoints = points.filter((p: PointData) => p.elevation != null)

  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Levelling Summary</title>${pageStyles()}</head><body>
  ${docHeader('Levelling Summary / Level Book Abstract', project, surveyorDetails)}
...
export function generateControlSubmission(
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Control Survey Report</title>${pageStyles()}</head><body>
  ${docHeader('Control Survey Report — Form C22', project, surveyorDetails)}
