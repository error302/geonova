...
export const GET = apiHandler({ auth: true, rateLimit: { max: 60, windowMs: 60000 } }, async (_req, ctx) => {
  ...
  const [pointsRes, fieldbookRes, computationsRes, deedPlanRes, submissionRes] = await Promise.all([
    ...
    db.query<CntRow>('SELECT COUNT(*)::int AS cnt FROM fieldbooks WHERE project_id = $1', [id]).catch(() => ({ rows: [{ cnt: 0 }] })),
    db.query<CntRow>('SELECT COUNT(*)::int AS cnt FROM network_adjustments WHERE project_id = $1', [id]).catch(() => ({ rows: [{ cnt: 0 }] })),
