...
export const PUT = apiHandler({ auth: true, rateLimit: { max: 60, windowMs: 60000 } }, async (req, ctx) => {
  ...
  const { rows } = await db.query<AlignmentIpsRow>(
...
async function computeIPValues(
  ...
  const { rows: alignRows } = await client.query<RoadAlignmentRow>(
  ...
  for (let i = 0; i < ips.length; i++) {
    ...
    const { rows: updatedRows } = await client.query<AlignmentIpsRow>(
      `UPDATE alignment_ips SET
        deflection_angle = $2,
        tangent_length = $3,
        arc_length = $4,
