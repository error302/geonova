...
/** find_nearby_beacons() — RETURNS TABLE per migration 042 (easting/northing numeric, distance_m added) */
interface NearbyBeaconRow {
  id: string
  beacon_number: string
  beacon_type: string
  easting: number
...
export const GET = apiHandler(
  ...
  async (req, ctx) => {
    ...
    if (easting && northing) {
      ...
      const result = await db.query<NearbyBeaconRow>(
    ...
    const result = await db.query<BeaconRegistryRow>(
...
export const POST = apiHandler(
  ...
  async (req, ctx) => {
    ...
    const result = await db.query<Pick<BeaconRegistryRow, 'id' | 'beacon_number' | 'easting' | 'northing' | 'created_at'>>(
