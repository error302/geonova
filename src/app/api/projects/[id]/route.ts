...
export async function DELETE(
  ...
  try {
    ...
    const ownership = await db.query<ProjectRow>(
      ...
      for (const table of UNIQUE_TABLES) {
        try {
          ...
          if (msg.includes('does not exist') || msg.includes('relation') || msg.includes('not exist')) {
            ...
            logger.warn(`[DELETE /api/projects/${id}] Failed to delete from ${table}:`,  { error: (err as Error).message })
    ...
    logger.error('[DELETE /api/projects] Error:', { error: error })
    return NextResponse.json(
      {
