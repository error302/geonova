    ...
    try {
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        try {
          ...
          const { rows } = await client.query<SurveyPointRow>(
...
export const GET = apiHandler(
  ...
  async (req, _ctx) => {
    ...
    // SECURITY (audit H-05, 2026-08-30): verify the caller owns or is a
    // member of the project before returning its points. Previously any
    // authenticated user could read any project's survey points by UUID.
    ...
    const { rows } = await db.query<SurveyPointRow>(sql, params)
