...
/** find_nearby_field_records() — RETURNS TABLE per migration 021 (parcel_numbers TEXT[], distance_m added) */
interface NearbyFieldRecordRow {
  id: string
  fr_number: string
  fr_type: string
  easting: number
...
export const GET = apiHandler(
  ...
  async (req, ctx) => {
    ...
    if (easting && northing) {
      ...
      const result = await db.query<NearbyFieldRecordRow>(
...
export const POST = apiHandler(
  ...
  async (req, ctx) => {
    ...
    const result = await db.query<Pick<FieldRecordRow, 'id' | 'fr_number' | 'contributed_at'>>(
