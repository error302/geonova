...
interface EquipmentCalibrationRow {
  id: string
  equipment_id: string
  user_id: string
...
export const GET = apiHandler(
  ...
  async (req, ctx) => {
    ...
    // Verify ownership
    const ownership = await db.query<EquipmentRow>(
    ...
    const result = await db.query<EquipmentCalibrationRow>(
    ...
    if (latest) {
      const next = new Date(String(latest.next_calibration_date))
      const now = new Date()
