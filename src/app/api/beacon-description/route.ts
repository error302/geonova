...
export const POST = apiHandler({ auth: true, rateLimit: { max: 60, windowMs: 60000 } }, async (req, ctx) => {
  ...
  if (body?.projectId || body?.parcelId) {
    ...
    if (parcelId) {
      ...
      const traverseResult = await db.query<TraverseCoordinateRow>(
      ...
      beacons = traverseResult.rows.map((row: TraverseCoordinateRow) => ({
      ...
      beacons = pointsResult.rows.map((row: SurveyPointRow) => ({
    ...
    const utmZone = project?.utm_zone ?? 37
    const hemisphere = project?.hemisphere || 'S'
    const datum = project?.datum || 'Arc 1960'
    ...
    const data: BeaconDescriptionData = {
      ...
      surveyorName: project?.surveyor_name || project?.user_full_name || '',
      iskNumber: project?.isk_number || '',
      firmName: project?.firm_name || '',
