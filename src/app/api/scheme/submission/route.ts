...
interface ParcelRow {
  ...
  lr_number_proposed: string
  area_ha: number
  status: string
  block_id: string
  block_number: string
  block_name: string
  // Not in the SELECT — read off the untyped row before typing; always undefined
  // at runtime so locality falls back to scheme.ward, matching prior behaviour.
  location?: string
}
...
export const GET = apiHandler({ auth: true, rateLimit: { max: 60, windowMs: 60000 } }, async (request, ctx) => {
  ...
  try { JSZip = (await import('jszip')).default as JSZipConstructor } catch {}
  ...
  for (const parcel of parcels) {
    try {
      const traverseCheck = await db.query<ParcelTraverseRow>(
      ...
      const coordsResult = await db.query<TraverseCoordinateRow>(
      ...
      const stations: StationPoint[] = coordsResult.rows.map((c) => ({
      ...
      const areaStr = parcel.area_ha
        ...
        : traverse.computed_area_ha ? `${toNum(traverse.computed_area_ha).toFixed(4)} Ha` : '—'

      const infoPairs: [string, string][] = [
        ['PARCEL:', toStr(parcel.parcel_number)],
        ['LR NO.:', toStr(parcel.lr_number_proposed) || 'Pending'],
        ['AREA:', areaStr],
        ['BLOCK:', `${toStr(parcel.block_number)} — ${toStr(parcel.block_name) || 'N/A'}`],
        ['ACCURACY:', toStr(traverse.accuracy_order) || '—'],
        ['PERIMETER:', traverse.total_perimeter ? `${toNum(traverse.total_perimeter).toFixed(3)} m` : '—'],
        ['DATUM:', 'ARC 1960'],
  ...
  try {
    ...
    for (const parcel of parcels) {
      const tc = await db.query<TraverseCoordinateRow>(
      ...
      if (tc.rows.length >= 3) {
        allParcelCoords.push(...tc.rows.map((c) => ({
