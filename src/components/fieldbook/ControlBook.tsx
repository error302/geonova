              ...
              {rows.map((r, idx) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td><input inputMode="decimal" className="input" value={r.instrumentHeight} onChange={(e) => setRows((p) => p.map((x) => (x.id === r.id ? { ...x, instrumentHeight: e.target.value } : x)))} /></td>
                    <td><input inputMode="decimal" className="input" value={r.targetHeight} onChange={(e) => setRows((p) => p.map((x) => (x.id === r.id ? { ...x, targetHeight: e.target.value } : x)))} /></td>
                    ...
                    <td><input inputMode="decimal" className="input font-mono" value={r.verticalAngle} onChange={(e) => setRows((p) => p.map((x) => (x.id === r.id ? { ...x, verticalAngle: e.target.value } : x)))} /></td>
                    <td><input inputMode="decimal" className="input font-mono" value={r.slopeDistance} onChange={(e) => setRows((p) => p.map((x) => (x.id === r.id ? { ...x, slopeDistance: e.target.value } : x)))} /></td>
