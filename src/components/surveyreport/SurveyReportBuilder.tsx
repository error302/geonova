...
export default function SurveyReportBuilder({ projectId, existingReportId }: SurveyReportBuilderProps) {
  ...
  const getCompletenessForSection = (section: ReportSection): number => {
    ...
    const gap = completeness.sectionsIncomplete.find(g => g.section === section)
  ...
  const renderSectionForm = () => {
    const sectionInfo = SECTION_INFO.find(s => s.section === activeSection)
    if (!sectionInfo) return null

    if (sectionInfo.autoGenerated) {
      const section = generatedSections.find(s => s.section === activeSection)
      return (
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-6">
              <h2 className="text-xl font-semibold">
                Section {SECTION_INFO.find(s => s.section === activeSection)?.number} — {SECTION_INFO.find(s => s.section === activeSection)?.title}
              </h2>
            </div>
            {renderSectionForm()}
          </div>
